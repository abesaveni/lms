...
public static class DbInitializer
{
    ...
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        var testUsers = new[]
        {
            new { Email = "admin1@liveexpert.ai", Username = "admin1", Password = "Admin@123", Role = UserRole.Admin, FirstName = "System", LastName = "Admin 1" },
            new { Email = "admin2@liveexpert.ai", Username = "admin2", Password = "Admin@123", Role = UserRole.Admin, FirstName = "System", LastName = "Admin 2" },
            new { Email = "tutor1@liveexpert.ai", Username = "tutor1", Password = "Tutor@123", Role = UserRole.Tutor, FirstName = "John", LastName = "Tutor 1" },
            new { Email = "tutor2@liveexpert.ai", Username = "tutor2", Password = "Tutor@123", Role = UserRole.Tutor, FirstName = "Sarah", LastName = "Tutor 2" },
            new { Email = "tutor3@liveexpert.ai", Username = "tutor3", Password = "Tutor@123", Role = UserRole.Tutor, FirstName = "Michael", LastName = "Tutor 3" },
            new { Email = "student1@liveexpert.ai", Username = "student1", Password = "Student@123", Role = UserRole.Student, FirstName = "Priya", LastName = "Student 1" },
            new { Email = "student2@liveexpert.ai", Username = "student2", Password = "Student@123", Role = UserRole.Student, FirstName = "Rahul", LastName = "Student 2" },
            new { Email = "student3@liveexpert.ai", Username = "student3", Password = "Student@123", Role = UserRole.Student, FirstName = "Emma", LastName = "Student 3" }
        };
        ...
        foreach (var testUser in testUsers)
        {
            ...
            var existingUser = await context.Users.FirstOrDefaultAsync(u => u.Email == testUser.Email);
            ...
            if (existingUser == null)
            {
                ...
                // Add profile
                if (testUser.Role == UserRole.Tutor)
                {
                    var tutorProfile = await context.TutorProfiles.FirstOrDefaultAsync(tp => tp.UserId == newUser.Id);
                    if (tutorProfile == null)
                    {
                        context.TutorProfiles.Add(new TutorProfile
                        {
                            Id = Guid.NewGuid(),
                            UserId = newUser.Id,
                            Bio = "Professional Tutor bio placeholder",
                            HourlyRate = 500,
                ...
                else if (testUser.Role == UserRole.Student)
                {
                    var studentProfile = await context.StudentProfiles.FirstOrDefaultAsync(sp => sp.UserId == newUser.Id);
                    if (studentProfile == null)
                    {
                        ...
                        do
                        {
                            var prefix = testUser.Username.ToUpper().Substring(0, Math.Min(3, testUser.Username.Length));
                            referralCode = $"{prefix}{Guid.NewGuid().ToString("N").Substring(0, 6).ToUpper()}";
                            var exists = await context.StudentProfiles.AnyAsync(sp => sp.ReferralCode == referralCode);
                            isUnique = !exists;
                            attempts++;
                            
                            if (attempts > 100)
                            {
                                // Fallback: use GUID if can't generate unique code
                    ...
                    {
                        // Ensure existing profile has a referral code
                        if (string.IsNullOrEmpty(studentProfile.ReferralCode))
                        {
                            string referralCode;
                            bool isUnique = false;
                            var random = new Random();
                            ...
                            do
                            {
                                var prefix = testUser.Username.ToUpper().Substring(0, Math.Min(3, testUser.Username.Length));
                                referralCode = $"{prefix}{Guid.NewGuid().ToString("N").Substring(0, 6).ToUpper()}";
                                var exists = await context.StudentProfiles.AnyAsync(sp => sp.ReferralCode == referralCode && sp.Id != studentProfile.Id);
                                isUnique = !exists;
                                attempts++;
                            ...
                            context.StudentProfiles.Update(studentProfile);
                        }
                    }
                }
            ...
            else
            {
                ...
                existingUser.PasswordHash = BCrypt.Net.BCrypt.HashPassword(testUser.Password);
